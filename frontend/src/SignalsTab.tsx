...
export default function SignalsTab({ onExpired }: { onExpired: (message: string) => void }) {
  ...
  return <div className="signals-tab">
    <section className="panel scanner-settings" aria-labelledby="scanner-title">
      <div className="scanner-heading"><span className="square-icon orange"><Activity size={21} /></span><div><h2 id="scanner-title">Nifty 100 crossover scanner</h2><p>Find the latest EMA crossover for each stock, newest first.</p></div><span className="scanner-market"><span /> NSE EQUITY</span></div>
      <form onSubmit={generate}>
        ...
        <div className="scanner-hint"><Clock3 size={14} /><p>Completed candles only · Times in IST · Up to 100 stocks in alphabetical order. A full scan can take several minutes.</p></div>
    ...
    <section className="panel signals-results" aria-labelledby="results-title" aria-busy={scanning}>
      <div className="results-toolbar"><div><h2 id="results-title">Latest crossovers <span className="count">{result?.signals_found ?? 0}</span></h2><p>Most recent crossover per stock within the selected lookback.</p></div><div className="signal-filters"><label className="signal-search"><Search size={16} /><span className="sr-only">Search by ticker or company</span><input type="search" placeholder="Search ticker or company" value={search} onChange={event => setSearch(event.target.value)} /></label><label className="signal-filter"><ListFilter size={16} /><span className="sr-only">Crossover filter</span><select value={filter} onChange={event => setFilter(event.target.value as typeof filter)}><option>All</option><option>Bullish</option><option>Bearish</option></select></label></div></div>
      <div className="signals-table-scroll" tabIndex={0} role="region" aria-label="Ranked crossover results">
        <table className="signals-table"><caption className="sr-only">Nifty 100 crossovers ranked by candle start date and time in IST</caption><thead><tr><th scope="col">Rank</th><th scope="col">Ticker</th><th scope="col">Company</th><th scope="col">Crossover Type</th><th scope="col">Crossover Date</th><th scope="col">Time · IST</th><th scope="col" className="number">Close</th><th scope="col" className="number">EMA {shown.short_ema || '—'}</th><th scope="col" className="number">EMA {shown.long_ema || '—'}</th></tr></thead>
          <tbody>{rows.map(row => <tr key={row.ticker}><td className="rank-cell">{row.rank.toString().padStart(2, '0')}</td><th scope="row" className="ticker-cell">{row.ticker}</th><td className="company-cell" title={row.company}>{row.company}</td><td><span className={`crossover-badge ${row.crossover_type.toLowerCase()}`}>{row.crossover_type === 'Bullish' ? <ArrowUp size={13} /> : <ArrowDown size={13} />}{row.crossover_type}</span></td><td>{row.crossover_date}</td><td className="time-cell">{row.crossover_time}</td><td className="number close-cell">{price.format(row.close)}</td><td className="number">{price.format(row.short_ema)}</td><td className="number">{price.format(row.long_ema)}</td></tr>)}</tbody>
        </table>
      </div>
      {scanning ? <div className="signals-empty" role="status"><LoaderCircle size={29} className="spin" /><h3>Scanning Nifty 100 stocks...</h3><p>Checking completed candles for genuine crossovers.<br />You can switch dashboard tabs while this scan runs.</p><span className="scan-indicator" /></div>
        : !result ? <div className="signals-empty"><span className="empty-scan-icon"><ScanLine size={28} /></span><h3>Your next scan starts here</h3><p>Choose your EMA periods and timeframe,<br />then select Generate Signals.</p></div>
          : rows.length === 0 ? <div className="signals-empty" role="status"><Search size={27} /><h3>{result.stocks_analyzed === 0 ? 'No stocks could be analyzed' : result.signals_found === 0 ? 'No crossovers in this period' : 'No matching signals'}</h3><p>{result.stocks_analyzed === 0 ? 'Review the details above, check historical-data access, and try again.' : result.signals_found === 0 ? 'Try a longer lookback or a different timeframe.' : 'Try another ticker, company, or crossover filter.'}</p></div> : null}
